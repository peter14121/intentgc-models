...
class Node2VecFeatureTwoLayerIntentGCSageNetModel(object):
  ...
  def build_predict_user_graph(self):
    ...
    cur_user_1s_final_embedding = self.one_layer_user_gcn(cur_user_ids, cur_user_whole_embedding, "itemsim")
  ...
  def one_layer_item_gcn(self, cur_item_ids, cur_item_whole_embedding, edge_name, is_share): 
    with ctx.local():
      # 1-D neighbors
    ...
    with tf.variable_scope("one_layer_convolve_item"):
      if is_share == False:
        cur_item_final_embedding = self.full_connect(cur_item_1s_concate_embedding, [self.config.whole_emb_size*2, self.config.item_l2_size], [self.config.item_l2_size], tf.nn.relu, "one_layer_convolve_item")
      else:
        cur_item_final_embedding = self.get_shared_full_connect(cur_item_1s_concate_embedding, tf.nn.relu, "one_layer_convolve_item")

    return cur_item_final_embedding
    
  
  def two_layer_item_gcn(self, cur_item_1s_final_embedding, prefix_name, is_share):
    ...
    with ctx.local():
      # 1-D neighbors
      cur_item_2s_neighbor_ids = self.get_neighbor_ids(cur_item_1s_neighbor_ids)
    ...
    # raw net
    # [batch * neighbor_cnt, emb_size]
    cur_item_2s_neighbor_pool_embedding_plat = tf.reshape(cur_item_2s_neighbor_pool_embedding, [batch_size * self.config.neighbor_cnt, -1])
    cur_item_1s_neighbor_format_embedding_plat = tf.reshape(cur_item_1s_neighbor_format_embedding, [batch_size * self.config.neighbor_cnt, -1])
    
    cur_item_2s_concate_embedding_plat = tf.concat((cur_item_2s_neighbor_pool_embedding_plat, cur_item_1s_neighbor_format_embedding_plat), 1)
    with tf.variable_scope("one_layer_convolve_item"):
        cur_item_neighbor_final_embedding_plat = self.get_shared_full_connect(cur_item_2s_concate_embedding_plat, tf.nn.relu, "one_layer_convolve_item")

    # [batch, neighbor_cnt, emb_size]
    cur_item_neighbor_final_embedding = tf.reshape(cur_item_neighbor_final_embedding_plat, [batch_size, self.config.neighbor_cnt, -1])

    # [batch, emb_size]
    cur_item_neighbor_pool_embedding = tf.reduce_mean(cur_item_neighbor_final_embedding, axis = 1)
   
    # [batch,  emb_size]
    cur_item_1s_concate_embedding = tf.concat((cur_item_neighbor_pool_embedding, cur_item_1s_final_embedding), 1)
    with tf.variable_scope("two_layer_convolve_item"):
      if is_share == False:
        cur_item_final_embedding = self.full_connect(cur_item_1s_concate_embedding, [self.config.item_l2_size*2, self.config.item_l2_size], [self.config.item_l2_size], None, "two_layer_convolve_item")
      else:
        cur_item_final_embedding = self.get_shared_full_connect(cur_item_1s_concate_embedding, None, "two_layer_convolve_item")

    return cur_item_final_embedding
    

  def one_layer_user_gcn(self, cur_user_ids, cur_user_whole_embedding, edge_name): 
    with ctx.local():
      # 1-D neighbors
      cur_user_1s_neighbor_ids = self.get_neighbor_ids(cur_user_ids)
      cache_name = 'user_%s_neighbor_ids' % edge_name
    ...
    cache_name = 'user_%s_neighbor_feas' % edge_name
    ...
    with tf.variable_scope("one_layer_convolve_user"):
      cur_user_final_embedding = self.full_connect(cur_user_1s_concate_embedding, [self.config.whole_user_emb_size*2, self.config.user_l2_size], [self.config.user_l2_size], tf.nn.relu, "one_layer_convolve_user")
  ...
  def two_layer_user_gcn(self, cur_user_1s_final_embedding):
    ...
    with ctx.local():
      # 1-D neighbors
      cur_user_2s_neighbor_ids = self.get_neighbor_ids(cur_user_1s_neighbor_ids)
      
      cur_user_2s_neighbor_like_brand_package, cur_user_2s_neighbor_like_cate_package, cur_user_2s_neighbor_like_term_package, cur_user_2s_neighbor_pre_item_basic_package, cur_user_2s_neighbor_info_package, cur_user_2s_neighbor_pre_item_score_package = self.make_user_fea(cur_user_2s_neighbor_ids)
    
    cur_user_2s_neighbor_whole_embedding = self.make_user_embedding(cur_user_2s_neighbor_like_brand_package, cur_user_2s_neighbor_like_cate_package, cur_user_2s_neighbor_like_term_package, cur_user_2s_neighbor_pre_item_basic_package, cur_user_2s_neighbor_info_package, cur_user_2s_neighbor_pre_item_score_package, share_embedding = True) 
    
    # reshape to [batch, neighbor_cnt, neighbor_cnt, emb_size]
    batch_size = tf.shape(cur_user_1s_neighbor_format_embedding)[0]
    cur_user_2s_neighbor_format_embedding = tf.reshape(cur_user_2s_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
 
    # [batch, neighbor_cnt, emb_size]
    cur_user_2s_neighbor_pool_embedding = tf.reduce_mean(cur_user_2s_neighbor_format_embedding, axis = 2)
    
    # raw net
    # [batch * neighbor_cnt, emb_size]
    cur_user_2s_neighbor_pool_embedding_plat = tf.reshape(cur_user_2s_neighbor_pool_embedding, [batch_size * self.config.neighbor_cnt, -1])
    cur_user_1s_neighbor_format_embedding_plat = tf.reshape(cur_user_1s_neighbor_format_embedding, [batch_size * self.config.neighbor_cnt, -1])
    
    cur_user_2s_concate_embedding_plat = tf.concat((cur_user_2s_neighbor_pool_embedding_plat, cur_user_1s_neighbor_format_embedding_plat), 1)
    with tf.variable_scope("one_layer_convolve_user"):
      cur_user_neighbor_final_embedding_plat = self.get_shared_full_connect(cur_user_2s_concate_embedding_plat, tf.nn.relu, "one_layer_convolve_user")
  
    # [batch, neighbor_cnt, emb_size]
    cur_user_neighbor_final_embedding = tf.reshape(cur_user_neighbor_final_embedding_plat, [batch_size, self.config.neighbor_cnt, -1])

    # [batch, emb_size]
    cur_user_neighbor_pool_embedding = tf.reduce_mean(cur_user_neighbor_final_embedding, axis = 1)
   
    # [batch,  emb_size]
    cur_user_1s_concate_embedding = tf.concat((cur_user_neighbor_pool_embedding, cur_user_1s_final_embedding), 1)
    with tf.variable_scope("two_layer_convolve_user"):
      cur_user_final_embedding = self.full_connect(cur_user_1s_concate_embedding, [self.config.user_l2_size*2, self.config.user_l2_size], [self.config.user_l2_size], None, "two_layer_convolve_user")
  ...
  def make_features_and_embedding(self):
    ...
    cur_item_1s_final_embedding = self.one_layer_item_gcn(cur_item_ids, cur_item_whole_embedding, "session", False)

    cur_item_two_layer_final_embedding = self.two_layer_item_gcn(cur_item_1s_final_embedding, '', False)
    ...
    cur_neg_item_1s_final_embedding = self.one_layer_item_gcn(cur_neg_item_ids, cur_neg_item_whole_embedding, "neg_session", True)
    
    cur_neg_item_two_layer_final_embedding = self.two_layer_item_gcn(cur_neg_item_1s_final_embedding, 'neg_', True)
