...
class Node2VecFeatureMetapath2Model(object):
  ...
  def build_train_graph(self):
    ...
    cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_user_1s_neighbor_pool_embedding = self.make_features_and_embedding()

    # 2. build network
    self.build_network(cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_user_1s_neighbor_pool_embedding)
  ...
  def build_test_graph(self):
    ...
    # 1. make item and user feature and embedding
    cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_user_1s_neighbor_poop_embedding = self.make_features_and_embedding()
  ...
  def cal_loss(self):
    ...
    if (self.item_l3.get_shape()[1] != self.item_1s_neighbor_l3.get_shape()[1]):
      print('fatal error! layer size is not match: 1s item')
      return False
    ...
    item_1s_neighbor_norm = tf.sqrt(tf.reduce_sum(tf.square(self.item_1s_neighbor_l3), axis=1))
    print('item norm shape:')
    print(item_norm.get_shape())
    user_norm = tf.sqrt(tf.reduce_sum(tf.square(self.user_l3), axis=1))
    user_1s_neighbor_norm = tf.sqrt(tf.reduce_sum(tf.square(self.user_1s_neighbor_l3), axis=1))
    ...
    # used for metapath++
    meta_mid_matrix = self.item_l3 * self.item_1s_neighbor_l3
    meta_neg_mid_matrix = self.neg_item_l3 * self.item_l3
    meta_z_out = tf.reduce_sum(meta_mid_matrix, axis=1, name="meta_z_out")
    meta_neg_z_out = tf.reduce_sum(meta_neg_mid_matrix, axis=1, name="meta_neg_z_out")
    self.meta_predicts = tf.divide(meta_z_out, tf.multiply(item_norm, item_1s_neighbor_norm) + 0.000001)
    self.meta_neg_predicts = tf.divide(meta_neg_z_out, tf.multiply(neg_item_norm, item_norm) + 0.000001)
    
    user_meta_mid_matrix = self.user_l3 * self.user_1s_neighbor_l3
    user_meta_neg_mid_matrix = self.neg_item_l3 * self.user_l3
    user_meta_z_out = tf.reduce_sum(user_meta_mid_matrix, axis=1, name="user_meta_z_out")
    user_meta_neg_z_out = tf.reduce_sum(user_meta_neg_mid_matrix, axis=1, name="user_meta_neg_z_out")
    self.user_meta_predicts = tf.divide(user_meta_z_out, tf.multiply(user_norm, user_1s_neighbor_norm) + 0.000001)
    self.user_meta_neg_predicts = tf.divide(user_meta_neg_z_out, tf.multiply(neg_item_norm, user_norm) + 0.000001)
    # used end
    ...
    meta_pre_loss = tf.maximum(0.0, self.meta_neg_predicts - self.meta_predicts + self.config.threshold)
    user_meta_pre_loss = tf.maximum(0.0, self.user_meta_neg_predicts - self.user_meta_predicts + self.config.threshold)
    ...
    self.loss = tf.reduce_sum(pre_loss, 0) + tf.reduce_sum(meta_pre_loss, 0) + tf.reduce_sum(user_meta_pre_loss, 0)
    
    return True
 

  def build_network(self, cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_user_1s_neighbor_pool_embedding):
    self.build_network_for_item(cur_item_whole_embedding)
    
    self.build_network_for_item_neighbor(cur_item_1s_neighbor_pool_embedding)
    
    self.build_network_for_user(cur_user_whole_embedding)
    
    self.build_network_for_user_neighbor(cur_user_1s_neighbor_pool_embedding)
  ...
  def build_network_for_item_neighbor(self, cur_item_1s_neighbor_whole_embedding):
    activation_fn = tf.nn.relu
    # build network for item
    self.item_1s_neighbor_l1 = self.get_shared_full_connect(cur_item_1s_neighbor_whole_embedding, activation_fn, "item_layer1")
    
    self.item_1s_neighbor_l2 = self.get_shared_full_connect(self.item_1s_neighbor_l1, activation_fn, "item_layer2")
    
    self.item_1s_neighbor_l3 = self.get_shared_full_connect(self.item_1s_neighbor_l2, None, "item_layer3")
  ...
  def build_network_for_user_neighbor(self, cur_user_1s_neighbor_whole_embedding):
    activation_fn = tf.nn.relu
    # build network for user
    self.user_1s_neighbor_l1 = self.get_shared_full_connect(cur_user_1s_neighbor_whole_embedding, activation_fn, "user_layer1")
    
    self.user_1s_neighbor_l2 = self.get_shared_full_connect(self.user_1s_neighbor_l1, activation_fn, "user_layer2")
    
    self.user_1s_neighbor_l3 = self.get_shared_full_connect(self.user_1s_neighbor_l2, None, "user_layer3")
  ...
  def one_layer_item_embedding(self, cur_item_ids): 
    with ctx.local():
      # 1-D neighbors
    ...
    return cur_item_1s_neighbor_pool_embedding
  ...
  def make_features_and_embedding(self):
    ...
    with ctx.local():
      cur_item_ids = tf.reshape(self.datas.mapped_item_id.embed_key.values, [-1])
      cur_walk1_item_ids, cur_walk2_item_ids = self.do_rand_walk(cur_item_ids, 2)
    ...
    #cur_item_1s_neighbor_pool_embedding = self.one_layer_item_embedding(cur_item_ids)
    cur_item_1s_neighbor_pool_embedding = cur_walk1_item_whole_embedding 

    # 2. make embedding for user
    with ctx.local():
      cur_user_ids = tf.reshape(self.datas.mapped_user_id.embed_key.values, [-1])
      cur_walk1_user_ids, cur_walk2_user_ids = self.do_rand_walk(cur_user_ids, 1)
      
      cur_user_like_brand_package, cur_user_like_cate_package, cur_user_like_term_package, cur_user_pre_item_basic_package, cur_user_info_package, cur_user_pre_item_score_package = self.make_user_fea(cur_user_ids)
      cur_walk1_user_like_brand_package, cur_walk1_user_like_cate_package, cur_walk1_user_like_term_package, cur_walk1_user_pre_item_basic_package, cur_walk1_user_info_package, cur_walk1_user_pre_item_score_package = self.make_user_fea(cur_walk1_user_ids)
    
    cur_user_whole_embedding = self.make_user_embedding(cur_user_like_brand_package, cur_user_like_cate_package, cur_user_like_term_package, cur_user_pre_item_basic_package, cur_user_info_package, cur_user_pre_item_score_package, share_embedding = False)
    cur_walk1_user_whole_embedding = self.make_user_embedding(cur_walk1_user_like_brand_package, cur_walk1_user_like_cate_package, cur_walk1_user_like_term_package, cur_walk1_user_pre_item_basic_package, cur_walk1_user_info_package, cur_walk1_user_pre_item_score_package, share_embedding = True)
    ...
    # 4. return
    return [cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_walk1_user_whole_embedding]
    
  
  def do_rand_walk(self, cur_ids, id_type):
    seq_len = 2

    # [batch, seq_len + 1]
    tgs_path = geops.random_walk(cur_ids, 2, walk_pattern=[id_type])
    # [seq_len + 1, batch]
    trans_tgs_path = tf.transpose(tgs_path)
    print(trans_tgs_path.get_shape())

    # [batch, 1]
    walk1_ids = tf.transpose(trans_tgs_path[1])

    # [batch, 1]
    walk2_ids = tf.transpose(trans_tgs_path[2])
 
    return [walk1_ids, walk2_ids]
