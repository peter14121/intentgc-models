...
class Node2VecFeatureTwoLayerIntentGCPlusModel(object):
  ...
  def build_predict_item_graph(self):
    ...
    cur_item_1s_final_embedding = self.one_layer_item_gcn(cur_item_ids, cur_item_whole_embedding, "session")

    cur_item_two_layer_final_embedding = self.two_layer_item_gcn(cur_item_1s_final_embedding, '')
    
    # 2. build item network
    self.build_network_for_item(cur_item_two_layer_final_embedding)
  ...
  def build_predict_user_graph(self):
    ...
    cur_user_1s_final_embedding = self.one_layer_user_gcn(cur_user_ids, cur_user_whole_embedding)
    
    cur_user_two_layer_final_embedding = self.two_layer_user_gcn(cur_user_1s_final_embedding)
    
    # 2. build user network
    self.build_network_for_user(cur_user_two_layer_final_embedding)
  ...
  def one_layer_item_gcn(self, cur_item_ids, cur_item_whole_embedding, edge_name): 
    with ctx.local():
      # 1-D neighbors
      cur_item_1s_neighbor_ids = self.get_neighbor_ids(cur_item_ids, 19)
      cache_name = 'item_user_neighbor_ids'
      self.cache_map[cache_name] = cur_item_1s_neighbor_ids
      
      cur_item_1s_cate_neighbor_ids = self.get_neighbor_ids(cur_item_ids, 20)
      cache_name = 'item_cate_neighbor_ids'
      self.cache_map[cache_name] = cur_item_1s_cate_neighbor_ids
      
      cur_item_1s_term_neighbor_ids = self.get_neighbor_ids(cur_item_ids, 21)
      cache_name = 'item_term_neighbor_ids'
      self.cache_map[cache_name] = cur_item_1s_term_neighbor_ids
      ...
      cur_item_1s_cate_neighbor_title_package, cur_item_1s_cate_neighbor_brand_package, cur_item_1s_cate_neighbor_cate_and_prop_package, cur_item_1s_cate_neighbor_basic_package, cur_item_1s_cate_neighbor_usual_user_level_package = self.make_item_fea(cur_item_1s_cate_neighbor_ids)
      cur_item_1s_term_neighbor_title_package, cur_item_1s_term_neighbor_brand_package, cur_item_1s_term_neighbor_cate_and_prop_package, cur_item_1s_term_neighbor_basic_package, cur_item_1s_term_neighbor_usual_user_level_package = self.make_item_fea(cur_item_1s_term_neighbor_ids)
    ...
    cur_item_1s_cate_neighbor_whole_embedding = self.make_shared_item_embedding(cur_item_1s_cate_neighbor_title_package, cur_item_1s_cate_neighbor_brand_package, cur_item_1s_cate_neighbor_cate_and_prop_package, cur_item_1s_cate_neighbor_basic_package, cur_item_1s_cate_neighbor_usual_user_level_package)
    cur_item_1s_term_neighbor_whole_embedding = self.make_shared_item_embedding(cur_item_1s_term_neighbor_title_package, cur_item_1s_term_neighbor_brand_package, cur_item_1s_term_neighbor_cate_and_prop_package, cur_item_1s_term_neighbor_basic_package, cur_item_1s_term_neighbor_usual_user_level_package)
    ...
    cache_name = 'item_user_neighbor_feas'
    self.cache_map[cache_name] = cur_item_1s_neighbor_format_embedding
    
    cur_item_1s_cate_neighbor_format_embedding = tf.reshape(cur_item_1s_cate_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, -1])
    cache_name = 'item_cate_neighbor_feas'
    self.cache_map[cache_name] = cur_item_1s_cate_neighbor_format_embedding
    
    cur_item_1s_term_neighbor_format_embedding = tf.reshape(cur_item_1s_term_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, -1])
    cache_name = 'item_term_neighbor_feas'
    self.cache_map[cache_name] = cur_item_1s_term_neighbor_format_embedding

    # [batch, emb_size]
    cur_item_1s_neighbor_pool_embedding = tf.reduce_mean(cur_item_1s_neighbor_format_embedding, axis = 1)
    cur_item_1s_cate_neighbor_pool_embedding = tf.reduce_mean(cur_item_1s_cate_neighbor_format_embedding, axis = 1)
    cur_item_1s_term_neighbor_pool_embedding = tf.reduce_mean(cur_item_1s_term_neighbor_format_embedding, axis = 1)

    cur_item_final_embedding = (cur_item_1s_neighbor_pool_embedding * self.convolve_weights_item_neighbor) + (cur_item_whole_embedding * self.convolve_weights_item_self) + (cur_item_1s_term_neighbor_pool_embedding * self.convolve_weights_item_term_neighbor) + (cur_item_1s_cate_neighbor_pool_embedding * self.convolve_weights_item_prop_neighbor) 
    
    return cur_item_final_embedding
    
  
  def two_layer_item_gcn(self, cur_item_1s_final_embedding, prefix_name):
    edge_name = '%ssession' % prefix_name
    cache_name = 'item_%s_neighbor_ids' % edge_name
    cur_item_1s_neighbor_ids = self.cache_map[cache_name]
   
    cache_name = 'item_%s_neighbor_feas' % edge_name
    cur_item_1s_neighbor_format_embedding = self.cache_map[cache_name]

    with ctx.local():
      # 1-D neighbors
      cur_item_2s_neighbor_ids = self.get_neighbor_ids(cur_item_1s_neighbor_ids, 19)
      
      cur_item_2s_neighbor_title_package, cur_item_2s_neighbor_brand_package, cur_item_2s_neighbor_cate_and_prop_package, cur_item_2s_neighbor_basic_package, cur_item_2s_neighbor_usual_user_level_package = self.make_item_fea(cur_item_2s_neighbor_ids)
    
    cur_item_2s_neighbor_whole_embedding = self.make_shared_item_embedding(cur_item_2s_neighbor_title_package, cur_item_2s_neighbor_brand_package, cur_item_2s_neighbor_cate_and_prop_package, cur_item_2s_neighbor_basic_package, cur_item_2s_neighbor_usual_user_level_package)
    
    # reshape to [batch, neighbor_cnt, neighbor_cnt, emb_size]
    batch_size = tf.shape(cur_item_1s_neighbor_format_embedding)[0]
    cur_item_2s_neighbor_format_embedding = tf.reshape(cur_item_2s_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
 
    # [batch, neighbor_cnt, emb_size]
    cur_item_2s_neighbor_pool_embedding = tf.reduce_mean(cur_item_2s_neighbor_format_embedding, axis = 2)
   
    # [batch, neighbor_cnt, emb_size]
    cur_item_neighbor_final_embedding = (cur_item_2s_neighbor_pool_embedding * self.convolve_weights_item_neighbor) + (cur_item_1s_neighbor_format_embedding * self.convolve_weights_item_self)
    
    # [batch, emb_size]
    cur_item_neighbor_pool_embedding = tf.reduce_mean(cur_item_neighbor_final_embedding, axis = 1)
   
    # [batch,  emb_size]
    cur_item_final_embedding = (cur_item_neighbor_pool_embedding * self.convolve_weights_item_2s_neighbor) + (cur_item_1s_final_embedding * self.convolve_weights_item_2s_self)
  ...
  def one_layer_user_gcn(self, cur_user_ids, cur_user_whole_embedding): 
    with ctx.local():
      # 1-D neighbors
      cur_user_1s_neighbor_ids = self.get_neighbor_ids(cur_user_ids, 19)
      cache_name = 'user_item_neighbor_ids'
      self.cache_map[cache_name] = cur_user_1s_neighbor_ids
      
      cur_user_1s_cate_neighbor_ids = self.get_neighbor_ids(cur_user_ids, 20)
      cache_name = 'user_cate_neighbor_ids'
      self.cache_map[cache_name] = cur_user_1s_cate_neighbor_ids
      
      cur_user_1s_term_neighbor_ids = self.get_neighbor_ids(cur_user_ids, 21)
      cache_name = 'user_term_neighbor_ids'
      self.cache_map[cache_name] = cur_user_1s_term_neighbor_ids
      ...
      cur_user_1s_cate_neighbor_like_brand_package, cur_user_1s_cate_neighbor_like_cate_package, cur_user_1s_cate_neighbor_like_term_package, cur_user_1s_cate_neighbor_pre_item_basic_package, cur_user_1s_cate_neighbor_info_package, cur_user_1s_cate_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_cate_neighbor_ids)
      cur_user_1s_term_neighbor_like_brand_package, cur_user_1s_term_neighbor_like_cate_package, cur_user_1s_term_neighbor_like_term_package, cur_user_1s_term_neighbor_pre_item_basic_package, cur_user_1s_term_neighbor_info_package, cur_user_1s_term_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_term_neighbor_ids)
    ...
    cur_user_1s_cate_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_cate_neighbor_like_brand_package, cur_user_1s_cate_neighbor_like_cate_package, cur_user_1s_cate_neighbor_like_term_package, cur_user_1s_cate_neighbor_pre_item_basic_package, cur_user_1s_cate_neighbor_info_package, cur_user_1s_cate_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_term_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_term_neighbor_like_brand_package, cur_user_1s_term_neighbor_like_cate_package, cur_user_1s_term_neighbor_like_term_package, cur_user_1s_term_neighbor_pre_item_basic_package, cur_user_1s_term_neighbor_info_package, cur_user_1s_term_neighbor_pre_item_score_package, share_embedding = True) 
    ...
    cache_name = 'user_item_neighbor_feas'
    self.cache_map[cache_name] = cur_user_1s_neighbor_format_embedding

    cur_user_1s_cate_neighbor_format_embedding = tf.reshape(cur_user_1s_cate_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, -1])
    cache_name = 'user_cate_neighbor_feas'
    self.cache_map[cache_name] = cur_user_1s_cate_neighbor_format_embedding
    
    cur_user_1s_term_neighbor_format_embedding = tf.reshape(cur_user_1s_term_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, -1])
    cache_name = 'user_term_neighbor_feas'
    self.cache_map[cache_name] = cur_user_1s_term_neighbor_format_embedding

    # [batch, emb_size]
    cur_user_1s_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_neighbor_format_embedding, axis = 1)
    cur_user_1s_cate_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_cate_neighbor_format_embedding, axis = 1)
    cur_user_1s_term_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_term_neighbor_format_embedding, axis = 1)

    cur_user_final_embedding = (cur_user_1s_neighbor_pool_embedding * self.convolve_weights_user_neighbor) + (cur_user_whole_embedding * self.convolve_weights_user_self) + (cur_user_1s_term_neighbor_pool_embedding * self.convolve_weights_user_term_neighbor) + (cur_user_1s_cate_neighbor_pool_embedding * self.convolve_weights_user_prop_neighbor) 
    return cur_user_final_embedding
  
  
  def two_layer_user_gcn(self, cur_user_1s_final_embedding):
    # reuse 1 order neighbor
    cache_name = 'user_item_neighbor_ids' 
    cur_user_1s_neighbor_ids = self.cache_map[cache_name]
    
    cache_name = 'user_term_neighbor_ids'
    cur_user_1s_term_neighbor_ids = self.cache_map[cache_name]
    
    cache_name = 'user_cate_neighbor_ids' 
    cur_user_1s_cate_neighbor_ids = self.cache_map[cache_name]
   
    # reuse 1 order neighbor fea
    cache_name = 'user_item_neighbor_feas'
    cur_user_1s_neighbor_format_embedding = self.cache_map[cache_name]
    
    cache_name = 'user_term_neighbor_feas'
    cur_user_1s_term_neighbor_format_embedding = self.cache_map[cache_name]
    
    cache_name = 'user_cate_neighbor_feas'
    cur_user_1s_cate_neighbor_format_embedding = self.cache_map[cache_name]

    with ctx.local():
      # 2-D neighbors 
      # 2s neighbors for 1s item neighbors
      cur_user_1s_item_2s_item_neighbor_ids = self.get_neighbor_ids(cur_user_1s_neighbor_ids, 19)
      cur_user_1s_item_2s_term_neighbor_ids = self.get_neighbor_ids(cur_user_1s_neighbor_ids, 20)
      cur_user_1s_item_2s_cate_neighbor_ids = self.get_neighbor_ids(cur_user_1s_neighbor_ids, 21)
      
      # 2s neighbors for 1s term neighbors
      cur_user_1s_term_2s_item_neighbor_ids = self.get_neighbor_ids(cur_user_1s_term_neighbor_ids, 19)
      cur_user_1s_term_2s_term_neighbor_ids = self.get_neighbor_ids(cur_user_1s_term_neighbor_ids, 20)
      cur_user_1s_term_2s_cate_neighbor_ids = self.get_neighbor_ids(cur_user_1s_term_neighbor_ids, 21)
      
      # 2s neighbors for 1s cate neighbors
      cur_user_1s_cate_2s_item_neighbor_ids = self.get_neighbor_ids(cur_user_1s_cate_neighbor_ids, 19)
      cur_user_1s_cate_2s_term_neighbor_ids = self.get_neighbor_ids(cur_user_1s_cate_neighbor_ids, 20)
      cur_user_1s_cate_2s_cate_neighbor_ids = self.get_neighbor_ids(cur_user_1s_cate_neighbor_ids, 21)
     
      # get 2s fea for 1s item neighbors
      cur_user_1s_item_2s_item_neighbor_like_brand_package, cur_user_1s_item_2s_item_neighbor_like_cate_package, cur_user_1s_item_2s_item_neighbor_like_term_package, cur_user_1s_item_2s_item_neighbor_pre_item_basic_package, cur_user_1s_item_2s_item_neighbor_info_package, cur_user_1s_item_2s_item_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_item_2s_item_neighbor_ids)
      cur_user_1s_item_2s_term_neighbor_like_brand_package, cur_user_1s_item_2s_term_neighbor_like_cate_package, cur_user_1s_item_2s_term_neighbor_like_term_package, cur_user_1s_item_2s_term_neighbor_pre_item_basic_package, cur_user_1s_item_2s_term_neighbor_info_package, cur_user_1s_item_2s_term_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_item_2s_term_neighbor_ids)
      cur_user_1s_item_2s_cate_neighbor_like_brand_package, cur_user_1s_item_2s_cate_neighbor_like_cate_package, cur_user_1s_item_2s_cate_neighbor_like_term_package, cur_user_1s_item_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_item_2s_cate_neighbor_info_package, cur_user_1s_item_2s_cate_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_item_2s_cate_neighbor_ids)
      
      # get 2s fea for 1s term neighbors
      cur_user_1s_term_2s_item_neighbor_like_brand_package, cur_user_1s_term_2s_item_neighbor_like_cate_package, cur_user_1s_term_2s_item_neighbor_like_term_package, cur_user_1s_term_2s_item_neighbor_pre_item_basic_package, cur_user_1s_term_2s_item_neighbor_info_package, cur_user_1s_term_2s_item_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_term_2s_item_neighbor_ids)
      cur_user_1s_term_2s_term_neighbor_like_brand_package, cur_user_1s_term_2s_term_neighbor_like_cate_package, cur_user_1s_term_2s_term_neighbor_like_term_package, cur_user_1s_term_2s_term_neighbor_pre_item_basic_package, cur_user_1s_term_2s_term_neighbor_info_package, cur_user_1s_term_2s_term_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_term_2s_term_neighbor_ids)
      cur_user_1s_term_2s_cate_neighbor_like_brand_package, cur_user_1s_term_2s_cate_neighbor_like_cate_package, cur_user_1s_term_2s_cate_neighbor_like_term_package, cur_user_1s_term_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_term_2s_cate_neighbor_info_package, cur_user_1s_term_2s_cate_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_term_2s_cate_neighbor_ids)
      
      # get 2s fea for 1s cate neighbors
      cur_user_1s_cate_2s_item_neighbor_like_brand_package, cur_user_1s_cate_2s_item_neighbor_like_cate_package, cur_user_1s_cate_2s_item_neighbor_like_term_package, cur_user_1s_cate_2s_item_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_item_neighbor_info_package, cur_user_1s_cate_2s_item_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_cate_2s_item_neighbor_ids)
      cur_user_1s_cate_2s_term_neighbor_like_brand_package, cur_user_1s_cate_2s_term_neighbor_like_cate_package, cur_user_1s_cate_2s_term_neighbor_like_term_package, cur_user_1s_cate_2s_term_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_term_neighbor_info_package, cur_user_1s_cate_2s_term_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_cate_2s_term_neighbor_ids)
      cur_user_1s_cate_2s_cate_neighbor_like_brand_package, cur_user_1s_cate_2s_cate_neighbor_like_cate_package, cur_user_1s_cate_2s_cate_neighbor_like_term_package, cur_user_1s_cate_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_cate_neighbor_info_package, cur_user_1s_cate_2s_cate_neighbor_pre_item_score_package = self.make_user_fea(cur_user_1s_cate_2s_cate_neighbor_ids)
   
    # get 2s embedding for 1s item neighbors
    cur_user_1s_item_2s_item_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_item_2s_item_neighbor_like_brand_package, cur_user_1s_item_2s_item_neighbor_like_cate_package, cur_user_1s_item_2s_item_neighbor_like_term_package, cur_user_1s_item_2s_item_neighbor_pre_item_basic_package, cur_user_1s_item_2s_item_neighbor_info_package, cur_user_1s_item_2s_item_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_item_2s_term_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_item_2s_term_neighbor_like_brand_package, cur_user_1s_item_2s_term_neighbor_like_cate_package, cur_user_1s_item_2s_term_neighbor_like_term_package, cur_user_1s_item_2s_term_neighbor_pre_item_basic_package, cur_user_1s_item_2s_term_neighbor_info_package, cur_user_1s_item_2s_term_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_item_2s_cate_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_item_2s_cate_neighbor_like_brand_package, cur_user_1s_item_2s_cate_neighbor_like_cate_package, cur_user_1s_item_2s_cate_neighbor_like_term_package, cur_user_1s_item_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_item_2s_cate_neighbor_info_package, cur_user_1s_item_2s_cate_neighbor_pre_item_score_package, share_embedding = True) 
    
    # get 2s embedding for 1s term neighbors
    cur_user_1s_term_2s_item_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_term_2s_item_neighbor_like_brand_package, cur_user_1s_term_2s_item_neighbor_like_cate_package, cur_user_1s_term_2s_item_neighbor_like_term_package, cur_user_1s_term_2s_item_neighbor_pre_item_basic_package, cur_user_1s_term_2s_item_neighbor_info_package, cur_user_1s_term_2s_item_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_term_2s_term_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_term_2s_term_neighbor_like_brand_package, cur_user_1s_term_2s_term_neighbor_like_cate_package, cur_user_1s_term_2s_term_neighbor_like_term_package, cur_user_1s_term_2s_term_neighbor_pre_item_basic_package, cur_user_1s_term_2s_term_neighbor_info_package, cur_user_1s_term_2s_term_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_term_2s_cate_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_term_2s_cate_neighbor_like_brand_package, cur_user_1s_term_2s_cate_neighbor_like_cate_package, cur_user_1s_term_2s_cate_neighbor_like_term_package, cur_user_1s_term_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_term_2s_cate_neighbor_info_package, cur_user_1s_term_2s_cate_neighbor_pre_item_score_package, share_embedding = True) 
    
    # get 2s embedding for 1s cate neighbors
    cur_user_1s_cate_2s_item_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_cate_2s_item_neighbor_like_brand_package, cur_user_1s_cate_2s_item_neighbor_like_cate_package, cur_user_1s_cate_2s_item_neighbor_like_term_package, cur_user_1s_cate_2s_item_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_item_neighbor_info_package, cur_user_1s_cate_2s_item_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_cate_2s_term_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_cate_2s_term_neighbor_like_brand_package, cur_user_1s_cate_2s_term_neighbor_like_cate_package, cur_user_1s_cate_2s_term_neighbor_like_term_package, cur_user_1s_cate_2s_term_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_term_neighbor_info_package, cur_user_1s_cate_2s_term_neighbor_pre_item_score_package, share_embedding = True) 
    cur_user_1s_cate_2s_cate_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_cate_2s_cate_neighbor_like_brand_package, cur_user_1s_cate_2s_cate_neighbor_like_cate_package, cur_user_1s_cate_2s_cate_neighbor_like_term_package, cur_user_1s_cate_2s_cate_neighbor_pre_item_basic_package, cur_user_1s_cate_2s_cate_neighbor_info_package, cur_user_1s_cate_2s_cate_neighbor_pre_item_score_package, share_embedding = True) 
   

    # reshape to [batch, neighbor_cnt, neighbor_cnt, emb_size] for 1s item neighbors
    batch_size = tf.shape(cur_user_1s_neighbor_format_embedding)[0]
    cur_user_1s_item_2s_item_neighbor_format_embedding = tf.reshape(cur_user_1s_item_2s_item_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_item_2s_term_neighbor_format_embedding = tf.reshape(cur_user_1s_item_2s_term_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_item_2s_cate_neighbor_format_embedding = tf.reshape(cur_user_1s_item_2s_cate_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    
    # reshape to [batch, neighbor_cnt, neighbor_cnt, emb_size] for 1s term neighbors
    batch_size = tf.shape(cur_user_1s_term_neighbor_format_embedding)[0]
    cur_user_1s_term_2s_item_neighbor_format_embedding = tf.reshape(cur_user_1s_term_2s_item_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_term_2s_term_neighbor_format_embedding = tf.reshape(cur_user_1s_term_2s_term_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_term_2s_cate_neighbor_format_embedding = tf.reshape(cur_user_1s_term_2s_cate_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    
    # reshape to [batch, neighbor_cnt, neighbor_cnt, emb_size] for 1s cate neighbors
    batch_size = tf.shape(cur_user_1s_cate_neighbor_format_embedding)[0]
    cur_user_1s_cate_2s_item_neighbor_format_embedding = tf.reshape(cur_user_1s_cate_2s_item_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_cate_2s_term_neighbor_format_embedding = tf.reshape(cur_user_1s_cate_2s_term_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
    cur_user_1s_cate_2s_cate_neighbor_format_embedding = tf.reshape(cur_user_1s_cate_2s_cate_neighbor_whole_embedding, [batch_size, self.config.neighbor_cnt, self.config.neighbor_cnt, -1])
 
    # [batch, neighbor_cnt, emb_size]
    cur_user_1s_item_2s_item_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_item_2s_item_neighbor_format_embedding, axis = 2)
    cur_user_1s_item_2s_term_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_item_2s_term_neighbor_format_embedding, axis = 2)
    cur_user_1s_item_2s_cate_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_item_2s_cate_neighbor_format_embedding, axis = 2)
    
    cur_user_1s_term_2s_item_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_term_2s_item_neighbor_format_embedding, axis = 2)
    cur_user_1s_term_2s_term_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_term_2s_term_neighbor_format_embedding, axis = 2)
    cur_user_1s_term_2s_cate_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_term_2s_cate_neighbor_format_embedding, axis = 2)
    
    cur_user_1s_cate_2s_item_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_cate_2s_item_neighbor_format_embedding, axis = 2)
    cur_user_1s_cate_2s_term_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_cate_2s_term_neighbor_format_embedding, axis = 2)
    cur_user_1s_cate_2s_cate_neighbor_pool_embedding = tf.reduce_mean(cur_user_1s_cate_2s_cate_neighbor_format_embedding, axis = 2)
   
    # [batch, neighbor_cnt, emb_size]
    cur_user_1s_item_neighbor_2s_final_embedding = (cur_user_1s_item_2s_item_neighbor_pool_embedding * self.convolve_weights_user_neighbor) + (cur_user_1s_neighbor_format_embedding * self.convolve_weights_user_self) + (cur_user_1s_item_2s_term_neighbor_pool_embedding * self.convolve_weights_user_term_neighbor) + (cur_user_1s_item_2s_cate_neighbor_pool_embedding * self.convolve_weights_user_prop_neighbor)
    cur_user_1s_term_neighbor_2s_final_embedding = (cur_user_1s_term_2s_item_neighbor_pool_embedding * self.convolve_weights_user_neighbor) + (cur_user_1s_term_neighbor_format_embedding * self.convolve_weights_user_self) + (cur_user_1s_term_2s_term_neighbor_pool_embedding * self.convolve_weights_user_term_neighbor) + (cur_user_1s_term_2s_cate_neighbor_pool_embedding * self.convolve_weights_user_prop_neighbor)
    cur_user_1s_cate_neighbor_2s_final_embedding = (cur_user_1s_cate_2s_item_neighbor_pool_embedding * self.convolve_weights_user_neighbor) + (cur_user_1s_cate_neighbor_format_embedding * self.convolve_weights_user_self) + (cur_user_1s_cate_2s_term_neighbor_pool_embedding * self.convolve_weights_user_term_neighbor) + (cur_user_1s_cate_2s_cate_neighbor_pool_embedding * self.convolve_weights_user_prop_neighbor)
    
    # [batch, emb_size]
    cur_user_1s_item_neighbor_2s_pool_embedding = tf.reduce_mean(cur_user_1s_item_neighbor_2s_final_embedding, axis = 1)
    cur_user_1s_term_neighbor_2s_pool_embedding = tf.reduce_mean(cur_user_1s_term_neighbor_2s_final_embedding, axis = 1)
    cur_user_1s_cate_neighbor_2s_pool_embedding = tf.reduce_mean(cur_user_1s_cate_neighbor_2s_final_embedding, axis = 1)
   
    # [batch,  emb_size]
    cur_user_final_embedding = (cur_user_1s_item_neighbor_2s_pool_embedding * self.convolve_weights_user_2s_neighbor) + (cur_user_1s_final_embedding * self.convolve_weights_user_2s_self) + (cur_user_1s_term_neighbor_2s_pool_embedding * self.convolve_weights_user_2s_term_neighbor) + (cur_user_1s_cate_neighbor_2s_pool_embedding * self.convolve_weights_user_2s_prop_neighbor)
  ...
  def create_convolve_weights(self):
    ...
    if self.ps_num > 1:
      ...
      self.convolve_weights_item_term_neighbor = tf.get_variable("convolve_weights_item_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_item_prop_neighbor = tf.get_variable("convolve_weights_item_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      ...
      self.convolve_weights_item_2s_neighbor = tf.get_variable("convolve_weights_item_2s_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_item_2s_self = tf.get_variable("convolve_weights_item_2s_self", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      ...
      self.convolve_weights_user_term_neighbor = tf.get_variable("convolve_weights_user_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_user_prop_neighbor = tf.get_variable("convolve_weights_user_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      ...
      self.convolve_weights_user_2s_neighbor = tf.get_variable("convolve_weights_user_2s_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_user_2s_term_neighbor = tf.get_variable("convolve_weights_user_2s_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_user_2s_prop_neighbor = tf.get_variable("convolve_weights_user_2s_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
      self.convolve_weights_user_2s_self = tf.get_variable("convolve_weights_user_2s_self", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev), partitioner=tf.min_max_variable_partitioner(max_partitions=self.ps_num))
    else:
      ...
      self.convolve_weights_item_term_neighbor = tf.get_variable("convolve_weights_item_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_item_prop_neighbor = tf.get_variable("convolve_weights_item_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      ...
      self.convolve_weights_user_term_neighbor = tf.get_variable("convolve_weights_user_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_user_prop_neighbor = tf.get_variable("convolve_weights_user_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      ...
      self.convolve_weights_item_2s_neighbor = tf.get_variable("convolve_weights_item_2s_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_item_2s_self = tf.get_variable("convolve_weights_item_2s_self", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_user_2s_neighbor = tf.get_variable("convolve_weights_user_2s_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_user_2s_term_neighbor = tf.get_variable("convolve_weights_user_2s_term_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_user_2s_prop_neighbor = tf.get_variable("convolve_weights_user_2s_prop_neighbor", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
      self.convolve_weights_user_2s_self = tf.get_variable("convolve_weights_user_2s_self", (1,), initializer=self.get_initializer(stddev=self.config.convolve_stddev))
  ...
  def make_features_and_embedding(self):
    ...
    cur_neg_item_1s_final_embedding = self.one_layer_item_gcn(cur_neg_item_ids, cur_neg_item_whole_embedding, "neg_session")
    
    cur_neg_item_two_layer_final_embedding = self.two_layer_item_gcn(cur_neg_item_1s_final_embedding, 'neg_')
 
    # 4. return
    return [cur_item_two_layer_final_embedding, cur_user_two_layer_final_embedding, cur_neg_item_two_layer_final_embedding]
    
  
  
  def get_neighbor_ids(self, cur_nodes_ids, column_id):
    # fill neighbor ids from graph
    src_feature_idxs = [column_id]
