...
class Node2VecFeatureDeepwalkModel(object):
  ...
  def build_train_graph(self):
    ...
    cur_item_whole_embedding, cur_walk1_item_whole_embedding, cur_walk2_item_whole_embedding, cur_neg_item_whole_embedding = self.make_features_and_embedding()

    # 2. build network
    self.build_network(cur_item_whole_embedding, cur_walk1_item_whole_embedding, cur_walk2_item_whole_embedding, cur_neg_item_whole_embedding)
  ...
  def build_predict_user_graph(self):
    ...
    with ctx.local():
      ...
      cur_user_like_cate_package, cur_user_like_brand_package, cur_user_like_term_package, cur_user_info_package, cur_user_pre_item_basic_package, cur_user_pre_shop_package, cur_user_pre_prop_package = self.make_user_fea(cur_user_ids)
    
    cur_user_whole_embedding = self.make_user_embedding(cur_user_like_cate_package, cur_user_like_brand_package, cur_user_like_term_package, cur_user_info_package, cur_user_pre_item_basic_package, cur_user_pre_shop_package, cur_user_pre_prop_package)
  ...
  def cal_loss(self):
    ...
    if (self.item_l3.get_shape()[1] != self.neg_item_l3.get_shape()[1]):
      ...
      return False
    if (self.item_l3.get_shape()[1] != self.walk1_item_l3.get_shape()[1]):
      print('fatal error! layer size is not match: neg')
      return False
    if (self.item_l3.get_shape()[1] != self.walk2_item_l3.get_shape()[1]):
      print('fatal error! layer size is not match: neg')
      return False
    
    mid_matrix = self.item_l3 * self.walk1_item_l3
    mid_matrix2 = self.walk1_item_l3 * self.walk2_item_l3
    print('mid matrix shape:')
    print(mid_matrix.get_shape())
    
    neg_mid_matrix = self.neg_item_l3 * self.item_l3
    neg_mid_matrix2 = self.neg_item_l3 * self.walk1_item_l3
    print('neg mid matrix shape:')
    print(neg_mid_matrix.get_shape())
    
    z_out = tf.reduce_sum(mid_matrix, axis=1, name="z_out")
    z_out2 = tf.reduce_sum(mid_matrix2, axis=1, name="z_out2")
    print('z_out shape:')
    print(z_out.get_shape())
    neg_z_out = tf.reduce_sum(neg_mid_matrix, axis=1, name="neg_z_out")
    neg_z_out2 = tf.reduce_sum(neg_mid_matrix2, axis=1, name="neg_z_out2")
    ...
    walk1_item_norm = tf.sqrt(tf.reduce_sum(tf.square(self.walk1_item_l3), axis=1))
    print('walk1 item norm shape:')
    print(walk1_item_norm.get_shape())
    walk2_item_norm = tf.sqrt(tf.reduce_sum(tf.square(self.walk2_item_l3), axis=1))
    print('walk2 item norm shape:')
    print(walk2_item_norm.get_shape())
    ...
    self.predicts = tf.divide(z_out, tf.multiply(item_norm, walk1_item_norm) + 0.000001)
    self.predicts2 = tf.divide(z_out2, tf.multiply(walk1_item_norm, walk2_item_norm) + 0.000001)
    print('predicts shape:')
    print(self.predicts.get_shape())
    self.neg_predicts = tf.divide(neg_z_out, tf.multiply(neg_item_norm, item_norm) + 0.000001)
    self.neg_predicts2 = tf.divide(neg_z_out2, tf.multiply(neg_item_norm, walk1_item_norm) + 0.000001)
    ...
    pre_loss2 = tf.maximum(0.0, self.neg_predicts2 - self.predicts2 + self.config.threshold)
    ...
    self.loss = tf.reduce_sum(pre_loss, 0) + tf.reduce_sum(pre_loss2, 0)
    
    return True
 

  def build_network(self, cur_item_whole_embedding, cur_walk1_item_whole_embedding, cur_walk2_item_whole_embedding, cur_neg_item_whole_embedding):
    self.build_network_for_item(cur_item_whole_embedding)
    
    self.build_network_for_walk1_item(cur_walk1_item_whole_embedding)
    self.build_network_for_walk2_item(cur_walk2_item_whole_embedding)
  ...
  def build_network_for_walk1_item(self, cur_walk1_item_whole_embedding):
    activation_fn = tf.nn.relu
    # build network for item
    self.walk1_item_l1 = self.get_shared_full_connect(cur_walk1_item_whole_embedding, activation_fn, "item_layer1")
    
    self.walk1_item_l2 = self.get_shared_full_connect(self.walk1_item_l1, activation_fn, "item_layer2")
    
    self.walk1_item_l3 = self.get_shared_full_connect(self.walk1_item_l2, None, "item_layer3")
  
  
  def build_network_for_walk2_item(self, cur_walk2_item_whole_embedding):
    activation_fn = tf.nn.relu
    # build network for item
    self.walk2_item_l1 = self.get_shared_full_connect(cur_walk2_item_whole_embedding, activation_fn, "item_layer1")
    
    self.walk2_item_l2 = self.get_shared_full_connect(self.walk2_item_l1, activation_fn, "item_layer2")
    
    self.walk2_item_l3 = self.get_shared_full_connect(self.walk2_item_l2, None, "item_layer3")
  ...
  def one_layer_item_gcn(self, cur_item_ids, cur_item_whole_embedding): 
    with ctx.local():
      ...
      cur_item_1s_neighbor_title_package, cur_item_1s_neighbor_cate_and_prop_package, cur_item_1s_neighbor_brand_package, cur_item_1s_neighbor_usual_user_basic_package, cur_item_1s_neighbor_usual_user_level_package, cur_item_1s_neighbor_shop_package, cur_item_1s_neighbor_basic_package = self.make_item_fea(cur_item_1s_neighbor_ids)
    
    cur_item_1s_neighbor_whole_embedding = self.make_shared_item_embedding(cur_item_1s_neighbor_title_package, cur_item_1s_neighbor_cate_and_prop_package, cur_item_1s_neighbor_brand_package, cur_item_1s_neighbor_usual_user_basic_package, cur_item_1s_neighbor_usual_user_level_package, cur_item_1s_neighbor_shop_package, cur_item_1s_neighbor_basic_package)
    ...
    cur_item_final_embedding = (cur_item_1s_neighbor_pool_embedding * self.convolve_weights_item_neighbor) + (cur_item_whole_embedding * self.convolve_weights_item_self)
  ...
  def one_layer_user_gcn(self, cur_user_ids, cur_user_whole_embedding): 
    with ctx.local():
      ...
      cur_user_1s_neighbor_like_cate_package, cur_user_1s_neighbor_like_brand_package, cur_user_1s_neighbor_like_term_package, cur_user_1s_neighbor_info_package, cur_user_1s_neighbor_pre_item_basic_package, cur_user_1s_neighbor_pre_shop_package, cur_user_1s_neighbor_pre_prop_package = self.make_user_fea(cur_user_1s_neighbor_ids)
    
    cur_user_1s_neighbor_whole_embedding = self.make_user_embedding(cur_user_1s_neighbor_like_cate_package, cur_user_1s_neighbor_like_brand_package, cur_user_1s_neighbor_like_term_package, cur_user_1s_neighbor_info_package, cur_user_1s_neighbor_pre_item_basic_package, cur_user_1s_neighbor_pre_shop_package, cur_user_1s_neighbor_pre_prop_package, share_embedding = True) 
    ...
    cur_user_final_embedding = (cur_user_1s_neighbor_pool_embedding * self.convolve_weights_user_neighbor) + (cur_user_whole_embedding * self.convolve_weights_user_self)
  ...
  def make_features_and_embedding(self):
    ...
    with ctx.local():
      ...
      cur_walk1_item_ids, cur_walk2_item_ids = self.do_rand_walk(cur_item_ids)

      cur_item_title_package, cur_item_brand_package, cur_item_cate_and_prop_package, cur_item_basic_package, cur_item_usual_user_level_package = self.make_item_fea(cur_item_ids)
      cur_walk1_item_title_package, cur_walk1_item_brand_package, cur_walk1_item_cate_and_prop_package, cur_walk1_item_basic_package, cur_walk1_item_usual_user_level_package = self.make_item_fea(cur_walk1_item_ids)
      cur_walk2_item_title_package, cur_walk2_item_brand_package, cur_walk2_item_cate_and_prop_package, cur_walk2_item_basic_package, cur_walk2_item_usual_user_level_package = self.make_item_fea(cur_walk2_item_ids)
    
    cur_item_whole_embedding = self.make_item_embedding(cur_item_title_package, cur_item_brand_package, cur_item_cate_and_prop_package, cur_item_basic_package, cur_item_usual_user_level_package)
    cur_walk1_item_whole_embedding = self.make_shared_item_embedding(cur_walk1_item_title_package, cur_walk1_item_brand_package, cur_walk1_item_cate_and_prop_package, cur_walk1_item_basic_package, cur_walk1_item_usual_user_level_package)
    cur_walk2_item_whole_embedding = self.make_shared_item_embedding(cur_walk2_item_title_package, cur_walk2_item_brand_package, cur_walk2_item_cate_and_prop_package, cur_walk2_item_basic_package, cur_walk2_item_usual_user_level_package)
    ...
    # 4. return
    return [cur_item_whole_embedding, cur_walk1_item_whole_embedding, cur_walk2_item_whole_embedding, cur_neg_item_whole_embedding]
    
 
  def do_rand_walk(self, cur_item_ids):
    seq_len = 2

    # [batch, seq_len + 1]
    tgs_path = geops.random_walk(cur_item_ids, 2, walk_pattern=[2])
    # [seq_len + 1, batch]
    trans_tgs_path = tf.transpose(tgs_path)
    print(trans_tgs_path.get_shape())

    # [batch, 1]
    walk1_item_ids = tf.transpose(trans_tgs_path[1])

    # [batch, 1]
    walk2_item_ids = tf.transpose(trans_tgs_path[2])
 
    return [walk1_item_ids, walk2_item_ids]
  ...
  # new imple, this will be a basic api that be called by various function
  def make_user_embedding(self, cur_user_like_cate_package, cur_user_like_brand_package, cur_user_like_term_package, cur_user_info_package, cur_user_pre_item_basic_package, cur_user_pre_shop_package, cur_user_pre_prop_package, share_embedding):
    ...
    cur_user_pre_shop_data = cur_user_pre_shop_package[0]
    cur_user_pre_shop_data_dim = cur_user_pre_shop_package[1]
    cur_user_pre_shop_data_name = cur_user_pre_shop_package[2]
    if share_embedding == True:
      cur_user_pre_shop_embedding = self.get_shared_sparse_embedding(cur_user_pre_shop_data, names=cur_user_pre_shop_data_name)
    else:
      cur_user_pre_shop_embedding = self.sparse_embedding(cur_user_pre_shop_data, cur_user_pre_shop_data_dim, self.config.shop_embedding_dim, names=cur_user_pre_shop_data_name)
    cur_user_pre_shop_embedding = tf.concat(cur_user_pre_shop_embedding, 1)
    
    cur_user_pre_prop_data = cur_user_pre_prop_package[0]
    cur_user_pre_prop_data_dim = cur_user_pre_prop_package[1]
    cur_user_pre_prop_data_name = cur_user_pre_prop_package[2]
    if share_embedding == True:
      cur_user_pre_prop_embedding = self.get_shared_sparse_embedding(cur_user_pre_prop_data, names=cur_user_pre_prop_data_name)
    else:
      cur_user_pre_prop_embedding = self.sparse_embedding(cur_user_pre_prop_data, cur_user_pre_prop_data_dim, self.config.cate_prop_embedding_dim, names=cur_user_pre_prop_data_name)
    cur_user_pre_prop_embedding = tf.concat(cur_user_pre_prop_embedding, 1)
    
    # 5. concate user all embeddings
    cur_user_whole_embedding = tf.concat([cur_user_like_cate_embedding, cur_user_like_brand_embedding, cur_user_like_term_embedding, cur_user_info_embedding, cur_user_pre_item_basic_embedding, cur_user_pre_shop_embedding, cur_user_pre_prop_embedding], 1)
  ...
  def normalize(self, input_vec):
    square = input_vec * input_vec
    norm = np.sqrt(np.sum(square, 1) )
    norm_vec = input_vec.T / norm
  ...
  def predict_user(self):
    ...
    while not self.sess.should_stop():
      ...
      norm_trained_vectors = self.normalize(trained_vectors)
      
      self.write_to_hdfs(f, cur_user_ids, norm_trained_vectors, self.config.output_hdfs_dir)
      cnt = cnt + 1
      if (cnt % 10000) == 0:
        print('%d write suc' % cnt)
  ...
  def predict_item(self):
    ...
    while not self.sess.should_stop():
      ...
      self.write_to_hdfs(f, cur_item_ids, norm_trained_vectors, self.config.output_hdfs_dir)
      cnt = cnt + 1
      if (cnt % 1000) == 0:
        print('%d write suc' % cnt)
