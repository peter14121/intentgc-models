...
class Node2VecFeatureBiNEModel(object):
  ...
  def cal_loss(self):
    ...
    if (self.user_l3.get_shape()[1] != self.user_1s_neighbor_l3.get_shape()[1]):
      print('fatal error! layer size is not match: 1s user')
      return False
    ...
    # just for laplass
    #laplass_item_loss = tf.reduce_mean(tf.square((tf.transpose(self.item_1s_neighbor_l3) / (item_1s_neighbor_norm + 0.000001)) - (tf.transpose(self.item_l3) / (item_norm + 0.000001))), 0)
    #laplass_user_loss = tf.reduce_mean(tf.square((tf.transpose(self.user_1s_neighbor_l3) / (user_1s_neighbor_norm + 0.000001)) - (tf.transpose(self.user_l3) / (user_norm + 0.000001))), 0)
    #real_laplass_item_loss = tf.transpose(laplass_item_loss)
    #real_laplass_user_loss = tf.transpose(laplass_user_loss)

    # new impl
    laplass_item_mid_matrix = self.item_l3 * self.item_1s_neighbor_l3
    laplass_user_mid_matrix = self.user_l3 * self.user_1s_neighbor_l3
    laplass_item_z_out = tf.reduce_sum(laplass_item_mid_matrix, axis=1, name="laplass_item_z_out")
    laplass_user_z_out = tf.reduce_sum(laplass_user_mid_matrix, axis=1, name="laplass_user_z_out")
    laplass_item_predicts = tf.divide(laplass_item_z_out, tf.multiply(item_norm, item_1s_neighbor_norm) + 0.000001)
    laplass_user_predicts = tf.divide(laplass_user_z_out, tf.multiply(user_norm, user_1s_neighbor_norm) + 0.000001)
    laplass_neg_item_mid_matrix = self.item_l3 * self.neg_item_l3
    laplass_neg_item_z_out = tf.reduce_sum(laplass_neg_item_mid_matrix, axis=1, name="laplass_neg_item_z_out")
    laplass_neg_item_predicts = tf.divide(laplass_neg_item_z_out, tf.multiply(item_norm, neg_item_norm) + 0.000001)
    ...
    pre_item_loss = tf.maximum(0.0, laplass_neg_item_predicts - laplass_item_predicts + self.config.auxi_threshold)
    pre_user_loss = tf.maximum(0.0, self.neg_predicts - laplass_user_predicts + self.config.auxi_threshold)
    ...
    self.loss = tf.reduce_sum(pre_loss, 0) + self.config.laplass_alpha * tf.reduce_sum(pre_item_loss, 0) + self.config.laplass_beta * tf.reduce_sum(pre_user_loss, 0)
  ...
  def one_layer_user_embedding(self, cur_user_ids): 
    with ctx.local():
      # 1-D neighbors
    ...
    return cur_user_1s_neighbor_pool_embedding
  ...
  def make_features_and_embedding(self):
    ...
    cur_user_1s_neighbor_pool_embedding = self.one_layer_user_embedding(cur_user_ids)
    ...
    # 4. return
    return [cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding, cur_item_1s_neighbor_pool_embedding, cur_user_1s_neighbor_pool_embedding]
