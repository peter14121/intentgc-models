...
class Node2VecFeatureModelByGraphPlatWithAPI(object):
  ...
  def build_predict_user_graph(self):
    ...
    cur_user_whole_embedding = self.make_user_embedding(cur_user_like_brand_package, cur_user_like_cate_package, cur_user_like_term_package, cur_user_pre_item_basic_package, cur_user_info_package, cur_user_pre_item_score_package)
  ...
  def cal_loss(self):
    ...
    self.test_auc, self.test_auc_op = tf.metrics.auc(labels_real_tensor, scores_tensor, name = 'test_auc3', num_thresholds = 2000)
  ...
  def make_features_and_embedding(self):
    ...
    cur_neg_item_whole_embedding = self.make_neg_item_embedding(cur_neg_item_title_package, cur_neg_item_brand_package, cur_neg_item_cate_and_prop_package, cur_neg_item_basic_package, cur_neg_item_usual_user_level_package)
 
    # 4. return
    return [cur_item_whole_embedding, cur_user_whole_embedding, cur_neg_item_whole_embedding]
  ...
  def make_neg_item_embedding(self, cur_neg_item_title_package, cur_neg_item_brand_package, cur_neg_item_cate_and_prop_package, cur_neg_item_basic_package, cur_neg_item_usual_user_level_package):
    # note here is no scope name, if there is several out api call, they will get the same weights
    
    # 2. make neg item embedding
    cur_neg_item_title_term_data = cur_neg_item_title_package[0]
    cur_neg_item_title_term_data_name = cur_neg_item_title_package[2]
    cur_neg_item_title_term_embedding = self.get_shared_sparse_embedding(cur_neg_item_title_term_data, names=cur_neg_item_title_term_data_name)
    cur_neg_item_title_term_embedding = tf.concat(cur_neg_item_title_term_embedding, 1)
    
    cur_neg_item_brand_data = cur_neg_item_brand_package[0]
    cur_neg_item_brand_data_name = cur_neg_item_brand_package[2]
    cur_neg_item_brand_embedding = self.get_shared_sparse_embedding(cur_neg_item_brand_data, names=cur_neg_item_brand_data_name)
    cur_neg_item_brand_embedding = tf.concat(cur_neg_item_brand_embedding, 1)
    
    cur_neg_item_cate_and_prop_data = cur_neg_item_cate_and_prop_package[0]
    cur_neg_item_cate_and_prop_data_name = cur_neg_item_cate_and_prop_package[2]
    cur_neg_item_cate_and_prop_embedding = self.get_shared_sparse_embedding(cur_neg_item_cate_and_prop_data, names=cur_neg_item_cate_and_prop_data_name)
    cur_neg_item_cate_and_prop_embedding = tf.concat(cur_neg_item_cate_and_prop_embedding, 1)
    
    # new add feature
    cur_neg_item_basic_data = cur_neg_item_basic_package[0]
    cur_neg_item_basic_data_name = cur_neg_item_basic_package[2]
    cur_neg_item_basic_embedding = self.get_shared_sparse_embedding(cur_neg_item_basic_data, names=cur_neg_item_basic_data_name)
    cur_neg_item_basic_embedding = tf.concat(cur_neg_item_basic_embedding, 1)
    
    cur_neg_item_usual_user_level_data = cur_neg_item_usual_user_level_package[0]
    cur_neg_item_usual_user_level_data_name = cur_neg_item_usual_user_level_package[2]
    cur_neg_item_usual_user_level_embedding = self.get_shared_sparse_embedding(cur_neg_item_usual_user_level_data, names=cur_neg_item_usual_user_level_data_name)
    cur_neg_item_usual_user_level_embedding = tf.concat(cur_neg_item_usual_user_level_embedding, 1)
    
    # 3. concate item all embeddings and dense
    #self.item_whole_embedding = tf.concat([self.item_title_term_embedding, self.item_cate_and_prop_embedding, self.item_brand_embedding, self.datas.i05.dense_val, self.datas.i06.dense_val, self.datas.i07.dense_val, self.datas.i08.dense_val], 1)
    cur_neg_item_whole_embedding = tf.concat([cur_neg_item_title_term_embedding, cur_neg_item_brand_embedding, cur_neg_item_cate_and_prop_embedding, cur_neg_item_basic_embedding, cur_neg_item_usual_user_level_embedding], 1)
    return cur_neg_item_whole_embedding
   

  def make_user_embedding(self, cur_user_like_brand_package, cur_user_like_cate_package, cur_user_like_term_package, cur_user_pre_item_basic_package, cur_user_info_package, cur_user_pre_item_score_package):
  ...
  def full_connect_sparse(self, train_inputs, weights_shape, sp_weights, scope_name):
    ...
    if self.ps_num > 1:
      ...
      weights = tf.get_variable("weights", weights_shape, initializer=self.get_initializer(stddev=self.config.embedding.stddev))
